...
public class CustomJMenuItem extends JMenuItem {
    ...
    final ImageIcon finalIconEmpty;
    final ImageIcon finalIconSelected;

    public CustomJMenuItem(String txt, String iconName, MainWindow parent) {
        super(txt);
        this.parent = parent;
        ...
        if (iconName != null) {
            ...
            try {
                iconEmpty = new ImageIcon(Objects.requireNonNull(getClass().getResource("/img/icons_32/empty.png")));
            } catch (Exception ne) {
                Log.f("no empty icon found " + iconName);
            }
    ...
    public void updateColors() {
        SwingUtilities.invokeLater(() -> {
            if (mouseInside && !isSelected()) {
                ...
                setIcon(finalIconSelected == null ? finalIconEmpty : finalIconSelected);
            } else if (isSelected()) {
                setIcon(finalIconSelected == null ? finalIconEmpty : finalIconSelected);
            } else {
                setIcon(finalIconDisabled == null ? finalIconEmpty : finalIconDisabled);
            }
        });
